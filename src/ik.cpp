...
class IK
{
    ...
    private: std_msgs::Float32MultiArray f32ma_msg;
    private: Eigen::Matrix<float, 6, 4> b, p;
    private: float height;

    public: IK(int argc, char **argv)
    {
        height = 2.0;
        b << -0.101,    0.8, 0.25, 1,
              0.101,    0.8, 0.25, 1,
        ...
        ros::NodeHandle nh;
        this->pub = nh.advertise<std_msgs::Float32MultiArray>("/stewart/piston_cmd", 100);
        ros::Subscriber sub = nh.subscribe("stewart/platform_twist", 100, &IK::callback, this);
        this->main();
    }
    ...
    private: void callback(const geometry_msgs::Twist::ConstPtr& msg)
    {
    ...
    private: Eigen::Matrix<float, 4, 4> transformation_matrix(float x, float y, float z, float r, float p, float yaw)
    {
